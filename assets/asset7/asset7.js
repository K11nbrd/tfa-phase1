...
function initCalc(){const form=qs('#asset7Calc'); if(!form) return; form.addEventListener('submit',e=>{e.preventDefault(); const fd=new FormData(form); const input={reason:fd.get('reason'), gender:fd.get('gender'), start:fd.get('start'), end:fd.get('end'), base:+fd.get('base')||0, food:+fd.get('food')||0, transport:+fd.get('transport')||0, fuelAnnual:+fd.get('fuelAnnual')||0}; renderResult(calc(input),input);});}
